  ...
  <section class="section">
    ...
    <div class="section-body">
      ...
      <div class="rule-actions">
        ...
        <div class="record-buttons"><button type="button" id="recordInput" class="record">● 入力を記録</button><button type="button" id="recordOutput" class="record">● 短押し時の実行内容を記録</button><button type="button" id="stopRecord">記録を中止</button></div>
      </div>
      ...
      <div class="subpanel" style="margin-top:12px">
        ...
        <div id="ruleEditor" class="editor-grid hidden">
          ...
          <div class="longpress-box">
            <label class="checkline"><input type="checkbox" id="ruleLongEnabled"> この割り当てで長押し判定を使う</label>
...
$('addRule').addEventListener('click',()=>ruleOp('add'));$('deleteRule').addEventListener('click',()=>ruleOp('delete'));$('duplicateRule').addEventListener('click',()=>ruleOp('duplicate'));$('ruleTop').addEventListener('click',()=>ruleOp('move',0));$('ruleUp').addEventListener('click',()=>ruleOp('move',null,-1));$('ruleDown').addEventListener('click',()=>ruleOp('move',null,1));$('ruleBottom').addEventListener('click',()=>ruleOp('move',999999));$('saveRule').addEventListener('click',saveRule);$('recordInput').addEventListener('click',()=>startRecord('record-input'));$('recordOutput').addEventListener('click',()=>startRecord('record-output'));$('recordLongOutput').addEventListener('click',()=>startRecord('record-long-output'));$('stopRecord').addEventListener('click',()=>action('record-stop'));$('testOutput').addEventListener('click',()=>action('test-output'));$('testLongOutput').addEventListener('click',()=>action('test-long-output'));$('ruleLongEnabled').addEventListener('change',()=>{markRuleDirty();updateLongPressEditor()});$('ruleLongAction').addEventListener('change',()=>{markRuleDirty();updateLongPressEditor()});['ruleEnabled','ruleInput','ruleOutput','ruleSuppress','rulePrefix','ruleLongMs','ruleLongOutput'].forEach(id=>$(id).addEventListener('input',markRuleDirty));
