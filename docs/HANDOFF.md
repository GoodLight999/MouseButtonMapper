...
- rc1製品実装commit: `8b4ec43e55daa319715ece7050d2c538a116e67c`
- rc2互換コントローラー実装の最終head・CI run: PR #5本文を正本とする
...
- Nintendo VID `0x057e` / Joy-Con（L）PID `0x2006`による純正識別
- Generic DesktopのGame Pad／Joystick HIDコレクション候補列挙
- 未知VID/PID互換品のStable ID明示選択とread-only互換モード
- XInput最大4台の自動検出・差分ポーリング
- 入力報告`0x30`、サブコマンド応答`0x21`、`0x3f`フォールバック、SDL互換7バイトinput-only形式
- 上下左右、L、ZL、SL、SR、－、キャプチャー、スティック押込み
- 左スティック12bit解析、正規化、キャリブレーション、デッドゾーン、ヒステリシス、4方向／8方向、軸反転
- Joy-Con／XInput＋マウス／キーボードの複合ルール
- Joy-Con／XInput短押し、長押し実行、長押しキャンセル、Hold出力
...
- 純正Bluetoothペアリングと入力報告モード`0x30`への切替
- 互換Raw HID候補選択、実report形式、全ボタンとスティックの照合
...
- Joy-Con／互換Raw HID I/Oは専用`JoyConWorker`で行う。
- XInputは専用`XInputWorker`で最大4スロットを監視し、フックコールバックへ処理を持ち込まない。
- 状態通知は接続・エラー・入力変化時を除き最大20Hz。
- HID／XInput切断時はDOWN中の全コントローラートークンへ合成UPを出す。
...
- HoldはJoy-ConまたはXInputの単独入力からキー出力に限定し、押下中だけキーを保持する。
- Hold参照カウントにより複数コントローラールールで同じキーを共有できる。
- コントローラー側のUPで、同じキーの物理押下を勝手に解放しない。
...
- `0x3f`は純正Joy-Conの簡易経路として扱い、SDL互換input-only形式は互換品のread-only経路として別解析する。
- HID capsのUsage Page/UsageでGame Pad／Joystickだけを候補化し、キーボード・マウス等を対象にしない。
- 未知VID/PIDを自動接続しない。ユーザーがStable IDを選択した場合だけ`ForcedCompatible`として開く。
- 未対応reportはreport長と先頭hexをエラーへ残す。
...
- Joy-Conは`Item{Kind:"JoyCon", Code:"ZL"}`、XInputは`Item{Kind:"XInput", Code:"P1:LB"}`等で保存する。
- 実行時は`InputToken`とDOWN/UPイベントへ正規化する。
- Joy-Con、XInput、マウス、キーボードは同じ押下集合とルール優先順位で評価する。
- キーボードを最後の入力とする複合ルールでも、前置コントローラー／マウス入力を消費済みにする。
...
- `joycon_app_windows.go`: Joy-Con subsystemと共通コントローラー入力への接続
- `controller_input_windows.go`: Joy-Con／XInput共通の記録、短押し、長押し、Hold、複合ルール
- `xinput_logic.go`: XInput状態差分、トリガー／スティックヒステリシス
- `xinput_windows.go`: XInput DLLロード、4スロットポーリング、切断処理
...
7. 実機不具合を同じブランチで修正し、必要なら`rc3`へ更新する。
